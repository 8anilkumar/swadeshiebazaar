...
import work.newproject.asus.`as`.swadeshiebazaar.ModelClass.ModelItems
...
class InvoiceActivity : AppCompatActivity() {
    ...
    var basfontLight: BaseFont = BaseFont.createFont("assets/fonts/app_font_light.ttf", "UTF-8", BaseFont.EMBEDDED)
    var appFontLight = Font(basfontLight, FONT_SIZE_SMALL)

    var basfontRegular: BaseFont = BaseFont.createFont("assets/fonts/app_font_regular.ttf", "UTF-8", BaseFont.EMBEDDED)
    var appFontRegular = Font(basfontRegular, FONT_SIZE_DEFAULT)

    var basfontSemiBold: BaseFont = BaseFont.createFont("assets/fonts/app_font_semi_bold.ttf", "UTF-8", BaseFont.EMBEDDED)
    var appFontSemiBold = Font(basfontSemiBold, 24f)

    var basfontBold: BaseFont = BaseFont.createFont("assets/fonts/app_font_bold.ttf", "UTF-8", BaseFont.EMBEDDED)
                    ...
                    .withListener(object : MultiplePermissionsListener {
                        override fun onPermissionsChecked(report: MultiplePermissionsReport) {
                            if (report.areAllPermissionsGranted()) {
                                ...
                                val outPath = getExternalFilesDir(null).toString() + "/my_invoice.pdf" //location where the pdf will store
                                Log.d("loc", outPath)
                                ...
                                data.add(ModelItems("jdnjk","fwef",1,33,3,67))
                                initBillDetails(doc)
                                addLine(writer)
                                initTableHeader(doc)
                                initItemsTable(doc)
                                ...
                                val path: Uri = FileProvider.getUriForFile(this@InvoiceActivity, this@InvoiceActivity.getPackageName() + ".provider", file);
                                try {
                                    ...
                                    Toast.makeText(this@InvoiceActivity, "There is no PDF Viewer", Toast.LENGTH_LONG).show()
                                }
                            } else {
                                Toast.makeText(this@InvoiceActivity, "Missing permissions", Toast.LENGTH_LONG).show()
                            }
                        }
    ...
    private fun initBillDetails(doc: Document) {
        ...
        val txtBilledToCell = PdfPCell(Phrase("Billed To", appFontLight))
        txtBilledToCell.border = Rectangle.NO_BORDER
        customerAddressTable.addCell(txtBilledToCell)
        appFontRegular.size = FONT_SIZE_DEFAULT
        appFontRegular.color = BaseColor.BLACK
        val clientAddressCell1 = PdfPCell(Paragraph("ANIL KUMAR PANDEY", appFontRegular))
        ...
        val clientAddressCell2 = PdfPCell(Paragraph("VIKAS NAGE", appFontRegular))
        ...
        val clientAddressCell3 = PdfPCell(Paragraph("NEAR PNB", appFontRegular))
        ...
        val clientAddressCell4 = PdfPCell(Paragraph("Address Line 3", appFontRegular))
    ...
    private fun initInvoiceHeader(doc: Document) {
        val d = resources.getDrawable(R.drawable.b)
        ...
        headerTable.setWidths(floatArrayOf(1.3f, 1f, 1f)) // adds 3 colomn horizontally
        ...
        val phoneCell = PdfPCell(Paragraph("+91 8547984369", appFontRegular))
        ...
        val line1 = PdfPCell(Paragraph("Address Line 1", appFontRegular))
